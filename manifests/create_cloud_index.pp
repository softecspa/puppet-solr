define solr::create_cloud_index (
  ...
  if !defined(Exec["update_zookeeper_conf_${index_type}"]) {
    exec {"update_zookeeper_conf_${index_type}":
      command     => "${solr_root}/solr-${solr_version}-scripts/zkcli.sh -z $zookeeper_addr -cmd upconfig -confdir /usr/local/etc/solr_xml_config/${index_type} -confname ${index_type}",
      refreshonly => true,
  ...
  exec {"upload_zookeeper_conf_${index_type}_${index}":
    command => "${solr_root}/solr-${solr_version}-scripts/zkcli.sh -z $zookeeper_addr -cmd upconfig -confdir /usr/local/etc/solr_xml_config/${index_type} -confname ${index_type}",
    unless  => "${solr_root}/solr-${solr_version}-scripts/zkcli.sh -z $zookeeper_addr/configs/${index_type} -cmd list 2> /dev/null",
